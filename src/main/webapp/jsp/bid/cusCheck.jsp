...
<head>
    ...
    <script type="text/javascript">
        ...
        function cusExpress(cusid){
        var objS = document.getElementById("cusSelect");
        var cusid = objS.options[objS.selectedIndex].value;
        	document.location.href="<%=basePath%>bidAction!cusExpress?cusid=" + cusid;
        }
        function keyExpress(kid){
        var objS = document.getElementById("keySelect");
        var kid = objS.options[objS.selectedIndex].value;
        	document.location.href="<%=basePath%>bidAction!keyExpress?kid=" + kid;
        }
    </script>
            ...
            <div class="row">
                ...
                <div class="col-lg-12">
                    ...
                    <form action="<%=basePath%>keyAction!addKey" method="post">
                        ...
                        <div class="row">
                        <div class="col-lg-1"><button type="button" class="btn btn-primary" onclick="javascript:window.location.href='<%=basePath%>bidAction!preCus'"
                         <c:if test="${indexCus<1}">value disabled</c:if> >Pre</button></div>
                        <div class="col-lg-1"><button type="button" class="btn btn-primary" onclick="javascript:window.location.href='<%=basePath%>bidAction!preKey'"
                        <c:if test="${indexKey<1}">value disabled</c:if> >Pre</button></div>
                        <div class="col-lg-1"><label class="control-label">客户</label></div>
                        <div class="col-lg-2"><span><select id="cusSelect" name="cusid" size="1" onchange="cusExpress()">
                                        <option value="${curCus.cusid}">${curCus.name}</option>
                                        <c:forEach items="${cuses}" var="cus" varStatus="status">
                                        <option value="${cus.cusid}">${cus.name}</option>
                                        </c:forEach>
                                    </select></span></div>
                        <div class="col-lg-1"><label class="control-label">关键字</label></div>
                        ...
                        <div class="col-lg-1"><button type="button" class="btn btn-success" onclick="javascript:window.location.href='<%=basePath%>bidAction!nextKey'"
                        <c:if test="${indexKey>=fn:length(myKeys)-1}">value disabled</c:if> >Next</button></div>
                        <div class="col-lg-1"><button type="button" class="btn btn-success" onclick="javascript:window.location.href='<%=basePath%>bidAction!nextCus'"
                        <c:if test="${indexCus>=fn:length(cuses)-1}">value disabled</c:if> >Next</button></div>
							...
							<td>【${status.count + pageSize * (currentPage - 1)}】</td>
									<td><label class="switch switch-danger">
										      <input type="checkbox" class="switch-input switch-cus" value="${con.cid }" <c:if test="${1 == con.delFlag}">checked</c:if>>
                        ...
                        <div class="row">
                                    ...
                                    <div class="col-lg-1"><button type="button" class="btn btn-primary preArt">Pre</button></div>
                             ...
                             <label class="switch switch-danger" >
										      <input type="checkbox" class="switch-input switch-cus switch-art" value="${curCon.cid }"
										       checked >
										      <span class="switch-label" data-on="On" data-off="Off"></span>
										      <span class="switch-handle"></span>
										    </label>
